    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            if self.hparams.data_type == 'binary':
                train_f1 = self.metrics(pred, targ)
...
###
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.hparams.data_type == 'binary':
            val_f1 = self.metrics(pred, targ)
        elif self.hparams.data_type == 'non-binary':
            targ_f1 = (targ > self.hparams.bin_thresh)
            pred_f1 = (pred > self.hparams.bin_thresh)
            val_f1 = self.metrics(pred_f1.float(), targ_f1.float())
        else:
            raise Exception(f'Invalid last activation given ({self.hparams.data_type}).')
